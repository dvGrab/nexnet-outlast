...
namespace CG
{
	...
	public:
		...
		unsigned char                                              UnknownData_UIYT[0x7];                                   // 0x0551(0x0007) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	public:
		void GetOwningPlayerCharacter(class APlayerCharacter** Return_Value);
		void ShowFireModes(class ABaseWeapon* Weapon);
		void ShowMagCheck();
		void OnWeaponFireModeChanged(EFireMode newFireMode);
		void TickCommandContext();
		class UMapActorWidget* AddMapActor(class UMapActorComponent* MapActorComponent, class UClass* MapActorIconWidgetClass, const struct FSlateBrush& IconBrush, const struct FLinearColor& IconColor, const struct FLinearColor& IconTextColor, float RotationOffset);
		...
		void IsCurvedHUDEnabled(bool* Return_Value);
