...
namespace CG
{
	...
	{
	public:
		struct FCoreUObject_FDateTime ToDateTime(const struct FZeuzTimestamp& T);
		struct FZeuzTimestamp Now(bool adjusted);
		bool IsZero(const struct FZeuzTimestamp& T);
		struct FZeuzTimestamp GetOffset();
		struct FZeuzTimestamp FromDateTime(const struct FCoreUObject_FDateTime& T, bool adjusted);
